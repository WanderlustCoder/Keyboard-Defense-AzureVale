...
const VIRTUAL_KEYBOARD_LAYOUT_ROWS = {
    qwerty: [
        ["q", "w", "e", "r", "t", "y", "u", "i", "o", "p"],
        ["a", "s", "d", "f", "g", "h", "j", "k", "l", ";"],
        ["z", "x", "c", "v", "b", "n", "m"]
...
const FINGER_LOOKUP_BY_LAYOUT = (() => {
    ...
    for (const [layoutId, rows] of Object.entries(VIRTUAL_KEYBOARD_LAYOUT_ROWS)) {
        const map = { ...PHYSICAL_FINGER_LOOKUP };
...
const MASTERY_CERTIFICATE_MILESTONE_SHOWN_KEY = "keyboard-defense:mastery-certificate-milestone-shown";
...
export class HudView {
    ...
    upgradePanel;
    practiceDummyPanel = null;
    practiceDummyStatus = null;
    ...
    tutorialProgressKey = null;
    tutorialHintDismissed = false;
    lastTutorialMessage = null;
    tutorialDock;
    tutorialDockStorageKey = "keyboard-defense:tutorial-dock-collapsed";
    tutorialDockCollapsed = false;
    tutorialDockStateKey = null;
    tutorialDockLabels = new Map();
    tutorialDockModal;
    virtualKeyboard;
    ...
    uiSoundSchemeState;
    musicStemState;
    wpmLadderHighlightTimeout = null;
    biomePanel;
    biomeOverlay;
    biomeState;
    biomeHighlightTimeout = null;
    ...
    certificateName = "";
    certificateStats;
    certificateDetailsCollapsed = true;
    mentorDialogue;
    mentorFocus = "neutral";
    mentorMessageCursor = {
        accuracy: 0,
    ...
    lastMilestoneAt = 0;
    milestoneCelebrationsDisabled = false;
    lastLessonMedalCelebratedId = null;
    lastLessonMilestoneCelebrated = 0;
    lessonMilestoneTrackingInitialized = false;
    lastCertificateCelebratedAt = null;
    masteryCertificateMilestoneShown = false;
    parentSummaryOverlay;
    parentSummary;
    castleSkin = "classic";
    parentalOverlayTrigger;
    dropoffOverlayTrigger;
    ...
    musicActiveLabel;
    postureReminder;
    postureReminderTimeout = null;
    postureReminderInterval = null;
    ...
    constructor(config, rootIds, callbacks) {
        ...
        const hudDockTabs = Array.from(document.querySelectorAll("#hud-dock-tabs .hud-dock-tab"));
        const hudPanes = Array.from(document.querySelectorAll(".hud-pane"));
        ...
        if (tutorialBannerElement instanceof HTMLElement) {
            ...
            const progressElement = tutorialBannerElement.querySelector("[data-role='tutorial-progress']");
            const closeElement = tutorialBannerElement.querySelector("[data-role='tutorial-close']");
            const skipElement = tutorialBannerElement.querySelector("[data-role='tutorial-skip']");
            if (toggleElement) {
        ...
        if (rootIds.tutorialDock) {
            ...
            const dockSummary = rootIds.tutorialDock.summary
                ...
                : null;
            if (dockContainer instanceof HTMLElement &&
                ...
                dockSteps.addEventListener("click", (event) => {
                    ...
                    const button = target?.closest("button[data-step-id]");
        ...
        if (rootIds.optionsOverlay) {
            ...
            const navButtons = optionsNav
                ...
                : [];
            const optionsMainColumn = optionsContainer?.querySelector(".options-main-column");
            const optionsPanelContainer = optionsContainer?.querySelector("#options-panels");
            const optionPanels = optionsPanelContainer
                ? Array.from(optionsPanelContainer.querySelectorAll(".options-panel"))
                : [];
            ...
            if (optionsContainer instanceof HTMLElement &&
                ...
                this.optionsOverlay = {
                    ...
                    musicLibraryButton: musicLibraryButton instanceof HTMLButtonElement ? musicLibraryButton : undefined,
                    musicLibrarySummary: musicLibrarySummary instanceof HTMLElement ? musicLibrarySummary : undefined,
                    uiSoundLibraryButton: uiSoundLibraryButton instanceof HTMLButtonElement ? uiSoundLibraryButton : undefined,
                    uiSoundLibrarySummary: uiSoundLibrarySummary instanceof HTMLElement ? uiSoundLibrarySummary : undefined,
                    uiSoundPreviewButton: uiSoundPreviewButton instanceof HTMLButtonElement ? uiSoundPreviewButton : undefined,
                    ...
                    sfxLibraryButton: sfxLibraryButton instanceof HTMLButtonElement ? sfxLibraryButton : undefined,
                    sfxLibrarySummary: sfxLibrarySummary instanceof HTMLElement ? sfxLibrarySummary : undefined,
                    ...
                    readabilityGuideButton: readabilityGuideButton instanceof HTMLButtonElement ? readabilityGuideButton : undefined,
                    ...
                    biomeGalleryButton: biomeGalleryButton instanceof HTMLButtonElement ? biomeGalleryButton : undefined,
                    trainingCalendarButton: trainingCalendarButton instanceof HTMLButtonElement ? trainingCalendarButton : undefined,
                    loreScrollsButton: loreScrollsButton instanceof HTMLButtonElement ? loreScrollsButton : undefined,
                    postureChecklistButton: postureChecklistButton instanceof HTMLButtonElement ? postureChecklistButton : undefined,
                    postureChecklistSummary: postureChecklistSummary instanceof HTMLElement ? postureChecklistSummary : undefined,
                    ...
                    virtualKeyboardLayoutSelect: virtualKeyboardLayoutSelect instanceof HTMLSelectElement
                    ...
                    accessibilityPresetToggle: accessibilityPresetToggle instanceof HTMLInputElement
                        ...
                        : undefined,
                    breakReminderIntervalSelect: breakReminderIntervalSelect instanceof HTMLSelectElement
                        ? breakReminderIntervalSelect
                        : undefined,
                    screenTimeGoalSelect: screenTimeGoalSelect instanceof HTMLSelectElement ? screenTimeGoalSelect : undefined,
                    screenTimeLockoutSelect: screenTimeLockoutSelect instanceof HTMLSelectElement ? screenTimeLockoutSelect : undefined,
                    screenTimeStatus: screenTimeStatus instanceof HTMLElement ? screenTimeStatus : undefined,
                    screenTimeResetButton: screenTimeResetButton instanceof HTMLButtonElement ? screenTimeResetButton : undefined,
                    ...
                    milestonePopupsToggle: milestonePopupsToggle instanceof HTMLInputElement ? milestonePopupsToggle : undefined,
                    ...
                    focusOutlineSelect: focusOutlineSelect instanceof HTMLSelectElement ? focusOutlineSelect : undefined,
                    castleSkinSelect: castleSkinSelect instanceof HTMLSelectElement ? castleSkinSelect : undefined,
                    dayNightThemeSelect: dayNightThemeSelect instanceof HTMLSelectElement ? dayNightThemeSelect : undefined,
                    parallaxSceneSelect: parallaxSceneSelect instanceof HTMLSelectElement ? parallaxSceneSelect : undefined,
                    ...
                    layoutPreviewButton: layoutPreviewButton instanceof HTMLButtonElement ? layoutPreviewButton : undefined,
                    fontScaleSelect,
                    ...
                    telemetryQueueDownloadButton: telemetryQueueDownloadButton instanceof HTMLButtonElement
                        ? telemetryQueueDownloadButton
                        : undefined,
                    telemetryQueueClearButton: telemetryQueueClearButton instanceof HTMLButtonElement ? telemetryQueueClearButton : undefined,
                    ...
                    endSessionButton: endSessionButton instanceof HTMLButtonElement ? endSessionButton : undefined,
                    analyticsExportButton: analyticsExportButton instanceof HTMLButtonElement ? analyticsExportButton : undefined,
                    sessionTimelineExportButton: sessionTimelineExportButton instanceof HTMLButtonElement
                        ? sessionTimelineExportButton
                        : undefined,
                    keystrokeTimingExportButton: keystrokeTimingExportButton instanceof HTMLButtonElement
                        ? keystrokeTimingExportButton
                        : undefined,
                    progressExportButton: progressExportButton instanceof HTMLButtonElement ? progressExportButton : undefined,
                    progressImportButton: progressImportButton instanceof HTMLButtonElement ? progressImportButton : undefined
                };
                ...
                if (postureReminder instanceof HTMLElement &&
                ...
                if (this.optionsOverlay.latencySparklineToggle) {
                    this.optionsOverlay.latencySparklineToggle.addEventListener("change", () => {
                        if (this.syncingOptionToggles)
                            return;
                        this.callbacks.onLatencySparklineToggle?.(this.optionsOverlay.latencySparklineToggle.checked);
                    });
                }
                ...
                if (this.optionsOverlay.audioNarrationToggle) {
                    this.optionsOverlay.audioNarrationToggle.addEventListener("change", () => {
                        ...
                        if (enabled) {
                            ...
                            this.narration.speak("Audio narration enabled. Menus and overlays will be spoken.", { interrupt: true });
                        }
                        else {
                ...
                if (this.optionsOverlay.focusOutlineSelect) {
                    this.optionsOverlay.focusOutlineSelect.addEventListener("change", () => {
                        if (this.syncingOptionToggles)
                            return;
                        const next = this.getSelectValue(this.optionsOverlay.focusOutlineSelect) ??
                            null;
                        if (next) {
                ...
                if (navButtons.length > 0 && panels.length > 0) {
                    const setActiveButton = (button) => {
                        navButtons.forEach((btn) => btn.setAttribute("aria-current", btn === button ? "true" : "false"));
                    };
                    ...
                    const activate = (panelKey) => {
                        const targetButton = navButtons.find((btn) => btn.dataset.target === panelKey) ?? navButtons[0];
                        setActiveButton(targetButton ?? null);
                        const resolvedKey = targetButton?.dataset.target ?? panelKey ?? panels[0]?.dataset.panel ?? "";
                    ...
                    const initialKey = navButtons.find((btn) => btn.getAttribute("aria-current") === "true")?.dataset
        ...
        if (rootIds.waveScorecard) {
            ...
            if (scorecardContainer instanceof HTMLElement &&
                ...
                this.waveScorecard = {
                    ...
                    coachList: isElementWithTag(scorecardCoachList, "ul")
                        ? scorecardCoachList
                        : undefined,
        ...
        if (rootIds.analyticsViewer) {
            ...
            const exportMeta = rootIds.analyticsViewer.exportMeta
                ? {
                    ...
                    ttf: document.getElementById(rootIds.analyticsViewer.exportMeta.timeToFirstTurret ?? ""),
            ...
            if (viewerContainer instanceof HTMLElement &&
                ...
                for (const button of tabButtons) {
                    const tabId = button.dataset.tab ?? null;
                ...
                if (summaryPanel instanceof HTMLElement)
                    panelMap.summary = summaryPanel;
                if (tracesPanel instanceof HTMLElement)
                    panelMap.traces = tracesPanel;
                if (exportsPanel instanceof HTMLElement)
                    panelMap.exports = exportsPanel;
                this.analyticsViewer = {
                    ...
                    exportMeta: exportMeta
                        ? {
                            ...
                            breaches: exportMeta.breaches instanceof HTMLElement ? exportMeta.breaches : undefined,
                ...
                if (this.analyticsViewer.tabs) {
                    ["summary", "traces", "exports"].forEach((tab) => {
        ...
        if (rootIds.dropoffOverlay) {
            ...
            if (dropoffContainer instanceof HTMLElement && dropoffClose instanceof HTMLButtonElement) {
                const reasonButtons = Array.from(dropoffContainer.querySelectorAll("button[data-dropoff-reason]")).filter((btn) => btn instanceof HTMLButtonElement);
                this.dropoffOverlay = {
        ...
        if (rootIds.musicOverlay) {
            ...
            if (musicContainer instanceof HTMLElement &&
        ...
        if (rootIds.uiSoundOverlay) {
            ...
            if (uiContainer instanceof HTMLElement &&
        ...
        if (rootIds.sfxOverlay) {
            ...
            if (sfxContainer instanceof HTMLElement &&
        ...
        if (rootIds.postureOverlay) {
            ...
            if (postureContainer instanceof HTMLElement &&
        ...
        if (rootIds.wpmLadderOverlay) {
            ...
            const ladderMeta = rootIds.wpmLadderOverlay.meta
                ...
                : null;
            if (ladderContainer instanceof HTMLElement &&
        ...
        if (rootIds.biomeOverlay) {
            ...
            const biomeMeta = rootIds.biomeOverlay.meta
                ...
                : null;
            if (biomeContainer instanceof HTMLElement &&
        ...
        if (rootIds.trainingCalendarOverlay) {
            ...
            const calLegend = rootIds.trainingCalendarOverlay.legend
                ...
                : null;
            if (calContainer instanceof HTMLElement &&
        ...
        if (rootIds.masteryCertificateOverlay) {
            ...
            if (certContainer instanceof HTMLElement && certClose instanceof HTMLButtonElement) {
                ...
                if (this.masteryCertificate.detailsToggle) {
                    this.masteryCertificate.detailsToggle.addEventListener("click", () => this.setMasteryCertificateDetailsCollapsed(!this.certificateDetailsCollapsed));
                }
    ...
    setVirtualKeyboardLayout(layout) {
    ...
    syncOptionsOverlayState(state) {
        ...
        if (this.optionsOverlay.musicToggle) {
            ...
            this.optionsOverlay.musicToggle.setAttribute("aria-disabled", musicDisabled ? "true" : "false");
        ...
        if (this.optionsOverlay.musicLevelSlider) {
            ...
            const level = typeof state.musicLevel === "number" && !Number.isNaN(state.musicLevel)
                ? state.musicLevel
                : 0.65;
        ...
        if (this.optionsOverlay.musicLibraryButton) {
            ...
            this.optionsOverlay.musicLibraryButton.setAttribute("aria-disabled", audioLibrariesDisabled ? "true" : "false");
        ...
        if (this.optionsOverlay.uiSoundLibraryButton) {
            ...
            this.optionsOverlay.uiSoundLibraryButton.setAttribute("aria-disabled", audioLibrariesDisabled ? "true" : "false");
        ...
        if (this.optionsOverlay.uiSoundPreviewButton) {
            ...
            this.optionsOverlay.uiSoundPreviewButton.setAttribute("aria-disabled", audioLibrariesDisabled ? "true" : "false");
        ...
        if (this.optionsOverlay.sfxLibraryButton) {
            ...
            this.optionsOverlay.sfxLibraryButton.setAttribute("aria-disabled", audioLibrariesDisabled ? "true" : "false");
        ...
        if (state.virtualKeyboardLayout) {
            ...
            if (this.optionsOverlay.virtualKeyboardLayoutSelect) {
                this.setSelectValue(this.optionsOverlay.virtualKeyboardLayoutSelect, state.virtualKeyboardLayout);
                const disabled = state.virtualKeyboardEnabled === false;
                this.optionsOverlay.virtualKeyboardLayoutSelect.disabled = disabled;
                this.optionsOverlay.virtualKeyboardLayoutSelect.setAttribute("aria-disabled", disabled ? "true" : "false");
        ...
        if (this.optionsOverlay.breakReminderIntervalSelect &&
            ...
            this.setSelectValue(this.optionsOverlay.breakReminderIntervalSelect, minutes <= 0 ? "off" : minutes.toString());
        }
        if (state.screenTime) {
            ...
            if (this.optionsOverlay.screenTimeGoalSelect) {
                ...
                this.setSelectValue(this.optionsOverlay.screenTimeGoalSelect, goalMinutes <= 0 ? "off" : goalMinutes.toString());
            }
            if (this.optionsOverlay.screenTimeLockoutSelect) {
            ...
            if (this.optionsOverlay.screenTimeStatus) {
                ...
                const goalMinutes = Math.max(0, Math.floor(screenTime.goalMinutes ?? 0));
                const base = goalMinutes > 0 ? `Today: ${minutesToday}/${goalMinutes} minutes` : `Today: ${minutesToday} minutes`;
                if (locked) {
            ...
            if (this.optionsOverlay.screenTimeResetButton) {
                ...
                this.optionsOverlay.screenTimeResetButton.setAttribute("aria-disabled", minutesToday <= 0 ? "true" : "false");
        ...
        const focusOutline = (state.focusOutlinePreset ?? this.focusOutlinePreset);
    ...
    update(state, upcoming, options = {}) {
        ...
        this.renderWavePreview(upcoming, options.colorBlindFriendly, state.laneHazards ?? [], options.wavePreviewEmptyMessage ?? null);
    ...
    getFingerMapping(char) {
        ...
        const layoutId = typeof this.virtualKeyboardLayout === "string" && this.virtualKeyboardLayout.length > 0
    ...
    persistTutorialDockCollapsed() {
        if (typeof window === "undefined" || !window.localStorage)
            return;
        try {
            window.localStorage.setItem(this.tutorialDockStorageKey, this.tutorialDockCollapsed ? "true" : "false");
        }
        catch {
            // ignore storage failures
        }
    }
    refreshTutorialDockLayout() {
        const dock = this.tutorialDock;
        if (!dock || dock.container.hidden)
            return;
    ...
    showTutorialDockModal(stepId) {
        if (!this.tutorialDockModal)
            return;
    ...
    setWavePreviewThreatIndicatorsEnabled(enabled) {
    ...
    updatePracticeDummyControls(state) {
        ...
        const spawnAvailable = running && !hasDummy && typeof this.callbacks.onPracticeDummySpawn === "function";
        ...
        if (this.practiceDummySpawnButton) {
            ...
            this.practiceDummySpawnButton.setAttribute("aria-disabled", spawnAvailable ? "false" : "true");
        }
        ...
        if (hasDummy) {
            ...
            const laneLabel = typeof laneValue === "number" ? `Lane ${String.fromCharCode(65 + laneValue)}` : "a lane";
    ...
    setWaveScorecardCoachField(field, label, value) {
        if (!this.waveScorecard?.coachList)
            return;
        const items = Array.from(this.waveScorecard.coachList.children);
        const target = items.find((element) => element.dataset.field === field);
        if (!target)
            return;
        const labelSpan = document.createElement("span");
    ...
    showDropoffOverlay(trigger) {
        if (!this.dropoffOverlay)
            return;
    ...
    updateMusicLevelDisplay(level) {
        if (!this.optionsOverlay?.musicLevelValue)
            return;
    ...
    setDayNightTheme(mode) {
        ...
        const select = this.optionsOverlay?.dayNightThemeSelect ??
            document.getElementById("options-day-night-theme");
        if (select) {
            this.setSelectValue(select, normalized);
        }
    }
    setParallaxScene(scene, resolved) {
        const normalized = scene === "day" || scene === "night" || scene === "storm" ? scene : "auto";
        ...
        const select = this.optionsOverlay?.parallaxSceneSelect ??
            document.getElementById("options-parallax-scene");
        if (select) {
            this.setSelectValue(select, normalized);
        }
    }
    resolveParallaxScene(scene) {
        if (scene === "day")
            return "day";
        if (scene === "storm")
            return "storm";
    ...
    applyParallaxScene(scene) {
        const resolved = scene === "storm" ? "storm" : scene === "day" ? "day" : "night";
    ...
    setParallaxMotionPaused(paused) {
    ...
    showLayoutOverlay() {
        ...
        const target = (this.hudLayoutSide === "left" ? this.layoutOverlay.leftApply : this.layoutOverlay.rightApply) ??
            focusable[0];
        target?.focus();
    }
    hideLayoutOverlay() {
    ...
    startPostureReminder(minutes = 5) {
        const durationMs = Math.max(1, minutes) * 60_000;
        this.clearPostureReminder();
        this.postureReminderTarget = Date.now() + durationMs;
        this.postureReminderTimeout = window.setTimeout(() => this.handlePostureReminderFired(), durationMs);
        this.postureReminderInterval = window.setInterval(() => this.updatePostureOverlayStatus(), 1000);
        const minutesLabel = Math.round(durationMs / 60_000);
        this.updatePostureOverlayStatus(`Reminder in ${this.formatPostureCountdown(this.postureReminderTarget)}.`);
        this.updatePostureSummary();
        this.appendLog(`Posture reminder set for ${minutesLabel} minute${minutesLabel === 1 ? "" : "s"}.`);
    }
    handlePostureReminderFired() {
        ...
        this.showPostureReminder("Quick posture reset: feet flat, wrists lifted, shoulders relaxed, screen at eye height.");
        this.updatePostureOverlayStatus("Reminder ready now.");
        this.updatePostureSummary();
    }
    clearPostureReminder(hideToast = true) {
    ...
    updatePostureOverlayStatus(message) {
        if (this.postureOverlay?.status) {
            if (message) {
                this.postureOverlay.status.textContent = message;
            }
            else if (this.postureReminderTarget) {
                this.postureOverlay.status.textContent = `Reminder in ${this.formatPostureCountdown(this.postureReminderTarget)}.`;
            }
            else if (this.postureLastReviewed) {
                this.postureOverlay.status.textContent = `Last reviewed ${this.describeElapsedTime(this.postureLastReviewed)}.`;
            }
            else {
    ...
    updatePostureSummary() {
        ...
        if (this.postureReminderTarget) {
            summary.textContent = `Posture reminder in ${this.formatPostureCountdown(this.postureReminderTarget)}.`;
            return;
        }
    ...
    readMasteryCertificateMilestoneShown() {
        if (typeof window === "undefined" || !window.localStorage)
            return false;
        try {
    ...
    readMilestoneCelebrationsDisabled() {
        if (typeof window === "undefined")
            return false;
        const read = (storage) => {
            if (!storage)
    ...
    persistMasteryCertificateMilestoneShown(shown) {
        if (typeof window === "undefined" || !window.localStorage)
            return;
        try {
            window.localStorage.setItem(MASTERY_CERTIFICATE_MILESTONE_SHOWN_KEY, shown ? "true" : "false");
        }
        catch {
            // ignore storage write failures
        }
    }
    persistMilestoneCelebrationsDisabled(disabled) {
        if (typeof window === "undefined")
            return;
        const write = (storage) => {
            if (!storage)
    ...
    celebrateMilestone(options) {
        if (!this.milestoneCelebration)
            return;
        if (this.milestoneCelebrationsDisabled)
            return;
        const reducedCognitiveLoad = this.hudRoot?.dataset.cognitiveMode === "reduced" ||
            (typeof document !== "undefined" && document.body?.dataset.cognitiveMode === "reduced");
        if (reducedCognitiveLoad)
            return;
        const now = Date.now();
    ...
    setSessionGoals(state) {
        ...
        if (!this.sessionGoalsPanel?.list)
            return;
        ...
        const items = shouldReplace
            ? []
            : Array.from(this.sessionGoalsPanel.list.querySelectorAll("li")).filter((node) => node instanceof HTMLLIElement);
    ...
    setDailyQuestBoard(state) {
        ...
        const items = shouldReplace
            ? []
            : Array.from(this.dailyQuestPanel.list.querySelectorAll("li")).filter((node) => node instanceof HTMLLIElement);
    ...
    setWeeklyQuestBoard(state) {
        ...
        const items = shouldReplace
            ? []
            : Array.from(this.weeklyQuestPanel.list.querySelectorAll("li")).filter((node) => node instanceof HTMLLIElement);
        for (let idx = 0; idx < nextEntries.length; idx += 1) {
            const entry = nextEntries[idx];
            const title = typeof entry?.title === "string" ? entry.title : "";
            const meta = typeof entry?.meta === "string" ? entry.meta : "";
    ...
    maybeCelebrateLessonMilestone(lessonsCompleted) {
        ...
        const nextThreshold = thresholds.find((value) => safeLessons >= value && this.lastLessonMilestoneCelebrated < value);
        if (!nextThreshold) {
    ...
    setLessonPathProgress(state) {
        this.lessonPathState = state;
        if (!this.lessonMedalPanel?.path)
            return;
    ...
    setLessonMedalProgress(state, options = {}) {
        const previousState = this.lessonMedalState ?? null;
        const previousLast = previousState?.last ?? null;
        const previousTimestamp = typeof previousLast?.timestamp === "number" && Number.isFinite(previousLast.timestamp)
        ...
        const lastTimestamp = typeof last?.timestamp === "number" && Number.isFinite(last.timestamp) ? last.timestamp : 0;
        const isNewResult = !!lastId &&
            (!!last &&
                (!previousLast ||
        ...
        if (celebrate && isNewResult && !alreadyCelebrated) {
            ...
            const previousBestForMode = mode && previousState?.bestByMode ? previousState.bestByMode[mode] ?? null : null;
            const previousModeBest = previousBestForMode ?? (previousLast?.mode === mode ? previousLast : null);
            const isTierUpgrade = !!last && (!previousModeBest || rankTier(last.tier) > rankTier(previousModeBest.tier));
            if (isTierUpgrade) {
                if (last.tier === "gold" || last.tier === "platinum") {
                    const tierLabel = last.tier.charAt(0).toUpperCase() + last.tier.slice(1).toLowerCase();
                    const modeLabel = last.mode === "burst"
                        ? "Burst"
                        : last.mode === "endurance"
                            ? "Endurance"
                            : last.mode === "sprint"
    ...
    setWpmLadder(state) {
        ...
        if (previous) {
            ...
            const improved = modes.some((mode) => {
                ...
                if (!after || !before)
                    return false;
                if (after.wpm > before.wpm)
                    return true;
                if (after.wpm === before.wpm && after.accuracy > before.accuracy)
                    return true;
                return false;
            });
            if (improved) {
    ...
    updateWpmLadderPanel(state) {
        if (!this.wpmLadderPanel)
            return;
    ...
    renderWpmLadderOverlay(state) {
        if (!this.wpmLadderOverlay)
            return;
        const safeState = state ?? this.getEmptyWpmLadderState();
    ...
    updateBiomePanel(state) {
        ...
        const active = safe.cards.find((card) => card.id === safe.activeId) ?? safe.cards[0] ?? null;
    ...
    renderBiomeOverlay(state) {
        if (!this.biomeOverlay)
            return;
        const safe = state ?? this.getEmptyBiomeState();
    ...
    updateTrainingCalendarPanel(state) {
        if (!this.trainingCalendarPanel)
            return;
    ...
    renderTrainingCalendarOverlay(state) {
        if (!this.trainingCalendarOverlay)
            return;
        const safe = state ?? this.getEmptyTrainingCalendarState();
    ...
    updateStreakTokenPanel() {
        if (!this.streakTokenPanel)
            return;
    ...
    setMasteryCertificateDetailsCollapsed(collapsed) {
    ...
    updateMusicActiveLabel(state) {
        if (!this.musicActiveLabel)
            return;
        const safe = state ?? this.getEmptyMusicState();
        const active = safe.entries.find((entry) => entry.active) ??
    ...
    renderMusicOverlay(state) {
        ...
        if (this.musicOverlay.summary) {
            const active = safe.entries.find((entry) => entry.active) ??
                safe.entries.find((entry) => entry.id === safe.activeId) ??
                null;
            const updated = safe.updatedAt && !Number.isNaN(Date.parse(safe.updatedAt))
                ? new Date(safe.updatedAt).toLocaleString()
                : "not yet refreshed";
    ...
    getEmptyUiSoundState() {
        return { activeId: "clarity", updatedAt: null, entries: [] };
    }
    updateUiSoundActiveLabel(state) {
        if (!this.uiSoundActiveLabel)
            return;
    ...
    getEmptySfxLibraryState() {
        return { activeId: "classic", updatedAt: null, entries: [] };
    }
    updateSfxActiveLabel(state) {
        if (!this.sfxActiveLabel)
            return;
    ...
    renderAnalyticsTraces(summaries, options = {}) {
    ...
    renderAnalyticsExportMeta(summaries, timeToFirstTurret) {
        ...
        const drillCount = Array.isArray(this.lastState?.analytics?.typingDrills)
            ...
            : 0;
        const breachCount = this.lastState?.analytics?.sessionBreaches ?? this.lastState?.analytics?.breaches ?? 0;
        if (waves) {
    ...
    setOptionsOverlayVisible(visible) {
        ...
        if (visible) {
            ...
            this.narration.speak("Options open. Tab through panels or press Escape to resume.", { interrupt: true });
        }
        else {
    ...
    updateSupportBoost(state) {
        if (!this.supportBoostBanner)
            return;
        const boost = state.supportBoost ??
            {
                lane: null,
                remaining: 0,
                duration: 0,
        ...
        const percentDelta = typeof boost.multiplier === "number" && Number.isFinite(boost.multiplier)
            ? Math.round((boost.multiplier - 1) * 100)
            : 0;
        const effectLabel = percentDelta !== 0
    ...
    formatFireRateEffect(multiplier) {

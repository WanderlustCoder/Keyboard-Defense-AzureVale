...
export class DiagnosticsOverlay {
    ...
    update(metrics, session) {
        ...
        const lines = [
            ...
            `Enemy HP x${difficulty.enemyHealthMultiplier.toFixed(2)} | Speed x${difficulty.enemySpeedMultiplier.toFixed(2)}`,
            ...
            `Wave damage (turret/typing/total): ${metrics.damage.turret.toFixed(1)} / ${metrics.damage.typing.toFixed(1)} / ${metrics.damage.total.toFixed(1)}`,
        ...
        if (defeatStats) {
            ...
            lines.push(`Defeat bursts: ${defeatStats.total} total (${perMinuteLabel}/min, sprite ${spritePctLabel}%)`);
            if (typeof defeatStats.lastTimestamp === "number" ||
                typeof defeatStats.lastEnemyType === "string") {
                const laneLabel = typeof defeatStats.lastLane === "number"
                    ? `lane ${defeatStats.lastLane + 1}`
                    : "lane ?";
                const ageLabel = typeof defeatStats.lastAgeSeconds === "number"
                    ? `${defeatStats.lastAgeSeconds.toFixed(1)}s ago`
                    : "n/a";
                const idleAlert = typeof defeatStats.lastAgeSeconds === "number" &&
                    defeatStats.lastAgeSeconds > 15 &&
                    metrics.enemiesAlive > 0
                    ? " ⚠️ idle"
                    : "";
                lines.push(`  Last: ${defeatStats.lastEnemyType ?? "unknown"} @ ${laneLabel} (${defeatStats.lastMode ?? "procedural"}, ${ageLabel})${idleAlert}`);
            }
        }
        const starfield = session?.starfield ?? null;
        if (starfield) {
            lines.push(`Starfield: drift ${formatFloat(starfield.driftMultiplier, 2)}x | depth ${formatFloat(starfield.depth, 2)} | tint ${starfield.tint}${starfield.reducedMotionApplied ? " (reduced motion)" : ""}`);
            lines.push(`  Wave ${Math.round(starfield.waveProgress * 100)}% · Castle HP ${Math.round(starfield.castleHealthRatio * 100)}% · Severity ${Math.round((starfield.severity ?? 0) * 100)}%`);
        ...
        if (assetIntegrity) {
            ...
            const totalImages = typeof assetIntegrity.totalImages === "number"
            ...
            lines.unshift(`Asset integrity: ${statusLabel} (${strictLabel}) - checked ${assetIntegrity.checked ?? 0}/${totalImages}${issueLabel}`);
            if (assetIntegrity.firstFailure) {
                const failure = assetIntegrity.firstFailure;
                const pathLabel = failure.path && failure.path !== failure.key ? `${failure.path} (${failure.key})` : failure.key;
                lines.splice(1, 0, `  First issue: ${pathLabel ?? "unknown"} [${failure.type}]`);
            }
        }
        else {
            lines.unshift("Asset integrity: telemetry unavailable (run npm run assets:integrity -- --check)");
        }
        ...
        if (recentGoldEvents.length > 0 && showGoldEvents) {
            ...
            const deltaLabel = `${deltaRounded >= 0 ? "+" : ""}${deltaRounded}g`;
            lines.push(`Recent gold events: latest ${deltaLabel} -> ${Math.round(latest.gold)}g (collapsed, tap expand to view)`);
        }
        if (castlePassives.length > 0) {
            if (showCastlePassives) {
                lines.push(`Castle passives (${castlePassives.length} active): ${castlePassives
                    .map(describeCastlePassive)
                    .join(" | ")}`);
            }
            else {
                lines.push(`Castle passives (${castlePassives.length} active) (collapsed, expand to view details)`);
            }
        }
        else {
        ...
        if (turretStats.length > 0 && showTurretStats) {
            ...
            for (const stat of turretStats) {
                const label = stat.turretType
                    ...
                    : "Empty";
                lines.push(`  ${stat.slotId}: ${label} - ${stat.damage.toFixed(1)} dmg | ${stat.dps.toFixed(1)} DPS`);
            }
        }
        else if (turretStats.length > 0) {
            const top = turretStats.reduce((prev, current) => (current.dps > prev.dps ? current : prev), turretStats[0]) ??
                null;
            const label = top?.turretType ? `${top.turretType.toUpperCase()} L${top.level ?? 1}` : "slot";
            lines.push(`Turret DPS: ${turretStats.length} tracked, top ${label} ${top?.dps.toFixed(1)} DPS (collapsed)`);
        }
        if (session) {
            ...
            if (session.lastSummary) {
                ...
                const averageReactionNote = typeof last.averageReaction === "number" && last.averageReaction > 0
                    ? ` | avg reaction ${last.averageReaction.toFixed(2)}s`
                    : "";
                lines.push(`Last wave ${last.index + 1}: ${last.enemiesDefeated} defeats, ${(last.accuracy * 100).toFixed(1)}% accuracy, ${last.breaches} breaches, DPS ${last.dps.toFixed(1)}, ${goldNote}${averageReactionNote}`);
                lines.push(`  Damage (turret/typing): ${turretDamage} / ${typingDamage} | DPS T/T: ${turretDpsStr} / ${typingDpsStr}`);
            }
        }
    ...
    setCanvasTransitionState(state) {
        if (this.canvasTransitionState === state)
            return;
    ...
    applySectionPreferences(preferences, options = {}) {
    ...
    setSectionCollapsed(sectionId, collapsed, options = {}) {
    ...
    renderSectionControls() {
        ...
        const controls = [];
        controls.push(`<button type="button" data-action="expand-all" class="diagnostics-control-button">Expand all</button>`);
        controls.push(`<button type="button" data-action="collapse-all" class="diagnostics-control-button">Collapse all</button>`);
        for (const sectionId of COLLAPSIBLE_SECTIONS) {
            const collapsed = this.isSectionCollapsed(sectionId);
            const label = SECTION_LABELS[sectionId];
            controls.push(`<button type="button" class="diagnostics-control-button" data-section="${sectionId}" aria-pressed="${collapsed ? "false" : "true"}">${collapsed ? "Show" : "Hide"} ${label}</button>`);
        }
        this.controlsRoot.innerHTML = `<div class="diagnostics-controls-inner">${controls.join("")}</div>`;
    }
}

...
export class GameController {
    ...
    handlePowerPhraseInput(input) {
        if (!input || !this.powerPhrasePhrase)
            return;
        ...
        const value = typeof input.value === "string" && input.value.length === 1
        ...
        const uiLocked = this.menuActive ||
            this.optionsOverlayActive ||
            this.waveScorecardActive ||
            this.tutorialHoldLoop;
        ...
        const lane = typeof this.hud?.getPowerPhraseLane === "function"
            ? this.hud.getPowerPhraseLane()
            : null;
        const resolvedLane = typeof lane === "number" && Number.isFinite(lane)
            ? lane
        ...
        if (applied) {
            ...
            this.hud?.appendLog?.(`Power phrase charged: lane ${laneLabel} +${boostPct}% for ${powerConfig.duration}s.`, "upgrade");
    ...
    updatePowerPhraseHud(state) {
        if (!this.hud?.updatePowerPhrase || !state)
            return;
        ...
        if (this.powerPhraseMessage && state.time < this.powerPhraseMessageUntil) {
            ...
            const laneLabel = typeof boost.lane === "number" ? LANE_LABELS[boost.lane] ?? `${boost.lane + 1}` : "a lane";
    ...
    replayTutorialStep(stepId) {
        if (!this.tutorialManager || !stepId)
            return;
        this.hud.hideTutorialSummary();
        this.pendingTutorialSummary = null;
        this.tutorialHoldLoop = false;

...
export class CanvasRenderer {
    ...
    drawTurretRangeHighlight(state, highlight) {
        if (!highlight) {
            return;
        }
        ...
        const levelConfig = archetype.levels.find((entry) => entry.level === level) ?? archetype.levels[0];
    ...
    withAlpha(hex, alpha) {
        ...
        const parse = (input) => Number.parseInt(input, 16);
        let r;
        let g;
        let b;
    ...
    drawEnemyWord(state, enemy, centerX, baselineY) {
        ...
        const totalWidth = widths.reduce((sum, width) => sum + width, 0) + letterSpacing * Math.max(0, word.length - 1);
    ...
    drawImpactEffects(state, effects) {
        if (effects.length === 0 || this.reducedMotion)
            return;
        const baseX = this.width * 0.1;
        for (const effect of effects) {
            ...
            const radius = effect.kind === "breach" ? 24 + 28 * (1 - effect.age) : 14 + 18 * (1 - effect.age);
            const innerColor = effect.kind === "breach" ? `rgba(248, 113, 113, ${alpha})` : `rgba(250, 204, 21, ${alpha})`;
            const outerColor = effect.kind === "breach" ? "rgba(248, 113, 113, 0)" : "rgba(250, 204, 21, 0)";
    ...
    drawCastle(state) {
        ...
        const drawn = this.assetLoader?.drawFrame?.(this.ctx, visual.spriteKey, x, y, castleWidth, castleHeight);
        if (drawn) {
    ...
    drawStatus(state) {
        ...
        if (state.wave.inCountdown) {
            ...
            this.ctx.fillText(`Prepare: ${state.wave.countdownRemaining.toFixed(1)}s`, this.width / 2, 48);
        }

...
} from "../tutorial/tutorialPersistence.js";
import {
  createDefaultPlayerSettings,
  readPlayerSettings as loadPlayerSettingsFromStorage,
...
import {
  LANE_MACRO_PRESETS,
  buildLanePriorityMap,
  getLaneMacroPreset
} from "../utils/laneMacros.js";
...
export class GameController {
  constructor(options) {
    ...
    this.hud = new HudView(
      ...
      },
      {
        ...
        onAccessibilitySelfTestConfirm: (kind, value) =>
          this.setAccessibilitySelfTestConfirmation(kind, value),
  ...
  setTurretDowngradeEnabled(enabled, options = {}) {
    ...
    if (!silent) {
      this.hud.appendLog(
        normalized ? "Turret downgrade mode enabled." : "Turret downgrade mode disabled."
      );
    }
    return normalized;
  }
  ...
  setColorblindPaletteEnabled(enabled, options = {}) {
    const nextMode =
      enabled && this.colorblindPaletteMode === "off"
        ? this.lastColorblindMode || "deuteran"
        : enabled
  ...
  setVoicePack(packId, options = {}) {
    ...
    if (this.voicePackId === next && !options.force) return this.voicePackId;
  ...
  normalizeHudZoom(value) {
    if (!Number.isFinite(value)) return HUD_ZOOM_DEFAULT;
  ...
  normalizeScreenShakeIntensity(value) {
    ...
    const clamped = Math.min(
      SCREEN_SHAKE_INTENSITY_MAX,
      Math.max(SCREEN_SHAKE_INTENSITY_MIN, value)
    );
    return Math.round(clamped * 100) / 100;
  }
    ...
    if (!enabled) {
      if (!options.silent) {
        ...
        if (queued > 0) {
          ...
          this.hud.appendLog(
            `Telemetry disabled; queue holds ${queued} ${noun}. Enable telemetry before flushing.`
          );
        } else {
  ...
  syncTelemetryDebugControls() {
    ...
    if (controls.flushButton) {
      ...
      controls.flushButton.setAttribute(
        "aria-disabled",
        controls.flushButton.disabled ? "true" : "false"
      );
    }
    if (controls.downloadButton) {
      ...
      controls.downloadButton.setAttribute(
        "aria-disabled",
        controls.downloadButton.disabled ? "true" : "false"
      );
      controls.downloadButton.setAttribute(
        "aria-label",
        queueLength === 1
  ...
  openTypingDrills(
    ...
    if (recommendation) {
      ...
      this.typingDrills.showNoRecommendation(
        "No recommendation available.",
        options?.autoStart && options.mode ? options.mode : null
      );
    }
  ...
  normalizeFieldDrillWord(word) {
    ...
    if (!trimmed || trimmed.length < FIELD_DRILL_MIN_WORD_LENGTH) return null;
    if (!FIELD_DRILL_WORD_REGEX.test(trimmed)) return null;
    return trimmed;
  }
  ...
  startFieldDrill(plan, options = {}) {
    if (!plan || this.fieldDrill) return false;
    ...
    const state = this.engine.getState();
    const heldEnemies =
      typeof this.engine.stashEnemies === "function"
  ...
  handleFieldDrillEnemyDefeated(enemy) {
    const drill = this.fieldDrill;
    if (!drill) return;
    if (!enemy || enemy.tierId !== "dummy") return;
  ...
  finishFieldDrill() {
    ...
    if (stats) {
      const summary: TypingDrillSummary = {
  ...
  computeFieldDrillStats(state) {
    ...
    const startClockMs =
      typeof drill.startClockMs === "number" && Number.isFinite(drill.startClockMs)
        ? drill.startClockMs
        : nowMs;
  ...
  applyFieldDrillRewards(drill, stats) {
    ...
    const spawnGateMultiplier =
      1 - (1 - tier.spawnGateMultiplier) * scale;
    ...
    if (supportApplied && typeof supportLane === "number") {
      ...
      rewardParts.push(
        `support lane ${laneLabel} +${boostPercent}% for ${supportSeconds.toFixed(1)}s`
      );
    }
  ...
  updateFieldDrillBuffs(state) {
    if (this.fieldDrillSpawnGateExpiresAt === null) return;
  ...
  resolveLaneMacroLaneToken(token, lanes) {
    ...
    if (normalized.length === 1) {
      ...
      if (code >= 97 && code <= 122) {
        const lane = code - 97;
        if (laneSet.has(lane)) return lane;
      }
    }
    const numeric = Number.parseInt(normalized, 10);
    if (Number.isFinite(numeric)) {
      if (laneSet.has(numeric)) return numeric;
      const zeroBased = numeric - 1;
      if (laneSet.has(zeroBased)) return zeroBased;
    }
    return null;
  }
  ...
  applyLaneMacroMap(map, options = {}) {
    ...
    const presetId =
      typeof options.presetId === "string" && options.presetId.length > 0
        ? options.presetId
  ...
  handleLaneMacroCommand(command) {
    ...
    if (head === "lane") {
      ...
      return applyMacro(
        nextMap,
        presetId,
        `${laneLabel} set to ${this.describeTargetingPriority(priority)}.`
      );
    }
    if (head === "lanes" || head === "all") {
      ...
      return applyMacro(
        map,
        presetId,
        `All lanes set to ${this.describeTargetingPriority(priority)}.`
      );
    }
  ...
  getLanePrioritySummary(state, lane) {
    const slots = (state?.turrets ?? []).filter(
      (slot) => slot.lane === lane && slot.unlocked
    );
    if (slots.length === 0) {
      return { priorityId: "locked", priorityLabel: "Locked" };
    }
    const priorities = new Set(
      slots.map((slot) => slot.targetingPriority ?? "first")
    );
  ...
  updateLaneMacroHud(state) {
    if (!this.hud?.updateLaneMacros || !state) return;
    const lanes = this.getLaneMacroLanes();
  ...
  trackMenuDrillQuickstart(
    recommendation: { mode: TypingDrillMode; reason?: string },
  ...
  persistPlayerSettings(patch) {
    ...
    const screenShakeEnabledUnchanged =
      patch.screenShakeEnabled === undefined ||
      patch.screenShakeEnabled === this.playerSettings.screenShakeEnabled;
    const screenShakeIntensityUnchanged =
      patch.screenShakeIntensity === undefined ||
      Math.abs(
        this.normalizeScreenShakeIntensity(patch.screenShakeIntensity) -
          this.playerSettings.screenShakeIntensity
      ) <= 0.001;
    ...
    const readableFontUnchanged =
      patch.readableFontEnabled === undefined ||
      patch.readableFontEnabled === this.playerSettings.readableFontEnabled;
    const dyslexiaFontUnchanged =
      patch.dyslexiaFontEnabled === undefined ||
      patch.dyslexiaFontEnabled === this.playerSettings.dyslexiaFontEnabled;
    const cognitiveLoadUnchanged =
      patch.reducedCognitiveLoadEnabled === undefined ||
    ...
    const voicePackUnchanged =
      patch.voicePackId === undefined || this.normalizeVoicePack(patch.voicePackId) === this.voicePackId;
    const accessibilityPresetUnchanged =
      patch.accessibilityPresetEnabled === undefined ||
    ...
    const hudZoomUnchanged =
      patch.hudZoom === undefined ||
      Math.abs(this.normalizeHudZoom(patch.hudZoom) - (this.playerSettings.hudZoom ?? HUD_ZOOM_DEFAULT)) <=
        0.001;
    const castleSkinUnchanged =
      patch.castleSkin === undefined ||
    ...
    const presetsUnchanged =
      patch.turretLoadoutPresets === undefined ||
      this.areTurretPresetMapsEqual(
        this.playerSettings.turretLoadoutPresets,
        patch.turretLoadoutPresets
      );
    const telemetryUnchanged =
      patch.telemetryEnabled === undefined ||
      patch.telemetryEnabled === this.playerSettings.telemetryEnabled;
    const currentSelfTest =
      this.playerSettings.accessibilitySelfTest ?? ACCESSIBILITY_SELF_TEST_DEFAULT;
    const nextSelfTestSnapshot =
      patch.accessibilitySelfTest === undefined
        ? currentSelfTest
        : {
            ...currentSelfTest,
            ...patch.accessibilitySelfTest,
            lastRunAt:
              patch.accessibilitySelfTest.lastRunAt === undefined
                ? currentSelfTest.lastRunAt
                : patch.accessibilitySelfTest.lastRunAt
          };
    const selfTestUnchanged = this.areAccessibilitySelfTestsEqual(
      currentSelfTest,
      nextSelfTestSnapshot
    );
  ...
  areTurretPresetMapsEqual(current = {}, next = {}) {
    ...
    for (const key of currentKeys) {
      ...
      if (!currentPreset && !nextPreset) continue;
      if (!currentPreset || !nextPreset) return false;
  ...
  areTurretPresetSlotsEqual(currentSlots = {}, nextSlots = {}) {
    ...
    for (const key of currentKeys) {
      ...
      if (
        currentSlot.typeId !== nextSlot.typeId ||
  ...
  cloneTurretPresetMap(source = {}) {
    const clone: TurretLoadoutPresetMap = Object.create(null);
  ...
  cloneTurretPresetSlots(slots = {}) {
    const clone: Record<string, TurretLoadoutSlot> = Object.create(null);
  ...
  captureTurretBlueprintFromState(state) {
    ...
    const blueprint: Record<string, TurretLoadoutSlot> = Object.create(null);
  ...
  syncTargetingPreferencesFromState(state) {
    const nextMap: Record<string, string> = Object.create(null);
    for (const slot of state.turrets) {
  ...
  syncTurretPresetsToHud(state) {
    ...
    const viewModels = TURRET_PRESET_IDS.map((presetId) =>
      this.buildTurretPresetViewModel(
        presetId,
        this.turretLoadoutPresets?.[presetId] ?? null,
        sourceState
      )
    );
  ...
  buildTurretPresetViewModel(presetId, preset, state) {
    ...
    const disabledTypes = Array.from(
      new Set(
  ...
  applySpawnSpeedGate() {
    const base =
      ...
      Number.isFinite(this.keystrokeTimingGateMultiplier)
        ...
        : 1;
    const drill =
      typeof this.fieldDrillSpawnGateMultiplier === "number" &&
  ...
  enqueueScreenShake(kind, options = {}) {
    if (this.reducedMotionEnabled) return;
    ...
    const intensity = this.normalizeScreenShakeIntensity(
      options.intensityOverride ?? this.screenShakeIntensity
    );
  ...
  getScreenShakeOffset() {
    if (this.reducedMotionEnabled) return null;
    const now = this.currentState?.time ?? this.engine.getState().time ?? 0;
    this.screenShakeBursts = this.screenShakeBursts.filter(
      (entry) => now - entry.createdAt <= entry.duration
    );
    if (this.screenShakeBursts.length === 0) return null;
    let magnitude = 0;
  ...
  render() {
    ...
    if (this.typingDrills?.isVisible?.()) {
      if (typingDrillRecommendation) {
        this.typingDrills.setRecommendation(
          typingDrillRecommendation.mode,
          typingDrillRecommendation.reason
        );
      } else {
    ...
    const summaries =
      analytics.waveHistory && analytics.waveHistory.length > 0
  ...
  sampleMemoryUsage() {
    ...
    const limitMB = performance.memory.jsHeapSizeLimit
      ...
      : null;
    const warning =
      typeof limitMB === "number" && limitMB > 0 ? usedMB / limitMB >= MEMORY_WARNING_RATIO : false;
    this.memorySample = {
      usedMB,
  ...
  attachFocusTrap() {
    ...
    const handler = (event) => {
      ...
      const modalOpen =
        this.hud.isOptionsOverlayVisible() ||
  ...
  initializePlayerSettings() {
    ...
    const initialColorblindMode =
      stored.colorblindPaletteEnabled === false
        ? "off"
        : this.colorblindPaletteMode !== "off"
          ? this.colorblindPaletteMode
          : this.lastColorblindMode ?? "deuteran";
    ...
    const storedTelemetry =
      typeof stored.telemetryEnabled === "boolean" ? stored.telemetryEnabled : false;
  ...
  attachDebugButtons() {
    ...
    const candidateTelemetryControls = {
      container:
        telemetryControlsContainer instanceof HTMLElement ? telemetryControlsContainer : null,
      ...
      endpointInput:
        telemetryEndpointInput instanceof HTMLInputElement ? telemetryEndpointInput : null,
      endpointApply:
        telemetryEndpointApply instanceof HTMLButtonElement ? telemetryEndpointApply : null
    };
    ...
    if (crystalToggleButton instanceof HTMLButtonElement) {
      ...
      crystalToggleButton.addEventListener("click", () =>
        this.setCrystalPulseEnabled(!this.featureToggles.crystalPulse)
      );
    } else {
      this.debugCrystalToggle = null;
    }
    ...
    if (downgradeToggleButton instanceof HTMLButtonElement) {
      ...
      downgradeToggleButton.addEventListener("click", () =>
        this.setTurretDowngradeEnabled(!this.featureToggles.turretDowngrade)
      );
    } else {
      this.debugDowngradeToggle = null;
    }
  ...
  handleTurretPresetSave(presetId) {
    ...
    const snapshot = this.captureTurretBlueprintFromState(
      this.currentState ?? this.engine.getState()
    );
    const nextPreset: TurretLoadoutPreset = {
      id: presetId,
  ...
  handleTurretPresetApply(presetId) {
    ...
    if (!preview.success) {
      this.hud.showCastleMessage(
        preview.message ?? `Unable to apply ${this.formatPresetLabel(presetId)}.`
      );
      return;
    }
    ...
    if (!result.success) {
      this.hud.showCastleMessage(
        result.message ?? `Unable to apply ${this.formatPresetLabel(presetId)}.`
      );
      return;
    }
    const state = this.engine.getState();
  ...
  buildLoreScrollViewState() {
    const summary = buildLoreScrollProgress(
      this.lessonProgress?.lessonsCompleted ?? 0,
      this.lessonProgress?.unlockedScrolls ?? []
    );
    return {
  ...
  handleLessonCompletion(summary: TypingDrillSummary) {
    ...
    const lessonId =
      summary?.mode === "lesson" && typeof summary.lessonId === "string" && summary.lessonId.length > 0
        ? summary.lessonId
        : null;
    ...
    const medalLabel =
      medalResult.record.tier.charAt(0).toUpperCase() + medalResult.record.tier.slice(1);
  ...
  registerHudListeners() {
    ...
    this.engine.events.on("enemy:spawned", (enemy) => {
      ...
      const displayed = this.hud.announceEnemyTaunt(
        `${enemyName} • ${laneLabel}: ${enemy.taunt}`
      );
      if (!displayed) {
    ...
    this.engine.events.on("turret:placed", (slot) => {
      this.hud.appendLog(
        `Turret deployed in ${slot.id.toUpperCase()} (${slot.turret?.typeId ?? "unknown"})`,
        "upgrade"
      );
    ...
    this.engine.events.on("analytics:wave-summary", (summary) => {
      ...
      const castleBonus =
        summary.castleBonusGold && summary.castleBonusGold > 0
      ...
      this.hud.appendLog(
        `Wave ${summary.index + 1} summary: ${summary.enemiesDefeated} defeats, ${(
  ...
  collectImpactEffects() {
    ...
    this.impactEffects = this.impactEffects.filter(
      (effect) => now - effect.createdAt <= MAX_LIFETIME
    );
  ...
  loadHotkeys() {
    ...
    try {
      ...
      const shortcuts =
        typeof parsed.shortcuts === "string" ? parsed.shortcuts.toLowerCase() : defaults.shortcuts;
  ...
  attachHotkeyControls() {
    ...
    const apply = () => {
      const pauseValue =
        pauseSelect instanceof HTMLSelectElement ? pauseSelect.value.toLowerCase() : "p";
      const shortcutsValue =
        shortcutsSelect instanceof HTMLSelectElement ? shortcutsSelect.value.toLowerCase() : "?";
  ...
  syncAccessibilityOverlay() {
    if (!this.accessibilityOverlay) return;
  ...
  maybeShowAccessibilityOnboarding() {
    if (
      this.accessibilityOnboardingSeen ||
      !this.accessibilityOverlay ||
  ...
  hideAccessibilityOnboarding() {
    if (!this.accessibilityOverlay || !this.accessibilityOverlay.visible) return;
    const overlay = this.accessibilityOverlay;
    overlay.container.dataset.visible = "false";
    overlay.container.setAttribute("aria-hidden", "true");
    overlay.visible = false;
    const shouldResume =
      this.resumeAfterAccessibility &&
      !this.running &&
      !this.optionsOverlayActive &&
      !this.typingDrillsOverlayActive &&
      !this.menuActive &&
  ...
  showEnemyIntroOverlay(tierId: string, bio: EnemyBiography) {
    if (!this.enemyIntroOverlay) return;
    ...
    const tipList =
      Array.isArray(bio.tips) && bio.tips.length > 0
        ? bio.tips
  ...
  hideEnemyIntroOverlay() {
    if (!this.enemyIntroOverlay || !this.enemyIntroOverlay.visible) return;
    const overlay = this.enemyIntroOverlay;
    overlay.container.dataset.visible = "false";
    overlay.container.setAttribute("aria-hidden", "true");
    overlay.visible = false;
  ...
  checkFirstEncounterOverlay() {
    if (!this.enemyIntroOverlay || this.enemyIntroOverlay.visible) return;
    if (this.optionsOverlayActive || this.typingDrillsOverlayActive || this.menuActive) return;
    if (!this.currentState?.enemies || this.currentState.enemies.length === 0) return;

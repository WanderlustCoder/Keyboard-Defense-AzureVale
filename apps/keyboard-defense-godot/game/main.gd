...
func _on_command_submitted(command: String) -> void:
	...
	if parsed.get("ok", false):
		...
		if is_night and typing_stats != null:
			if intent_kind == "defend_input":
				...
				# Show combo milestone message for major thresholds
				_check_combo_milestone(prev_combo, typing_stats.current_combo)
				# Check combo achievements when hitting thresholds
	...
	if command_bar != null and command_bar.has_method("show_error_feedback"):
		command_bar.show_error_feedback()
...
func _trigger_event_audio(events: Array) -> void:
		...
		for event in events:
				...
				# Exploration discovery
						...
						audio_manager.play_sfx(audio_manager.SFX.UI_CONFIRM)
				# Gathered resources
				elif text.begins_with("Gathered ") and text.contains("gold"):
						audio_manager.play_sfx(audio_manager.SFX.RESOURCE_PICKUP)
				# Tower attacks
				...
				# Building construction
						...
						audio_manager.play_sfx(audio_manager.SFX.BUILD_COMPLETE)
				# Upgrade purchase
				elif text.begins_with("Upgraded") or text.begins_with("Purchased"):
						audio_manager.play_sfx(audio_manager.SFX.UPGRADE_PURCHASE)
				# Night/wave start
				elif text.begins_with("Night falls"):
						audio_manager.play_sfx(audio_manager.SFX.WAVE_START)
				# POI discovery
				elif text.begins_with("POI discovered") or text.contains("point of interest"):
						audio_manager.play_sfx(audio_manager.SFX.POI_APPEAR)
...
func _trigger_event_visuals(events: Array) -> void:
		...
		for event in events:
				var text: String = str(event)
				# Enemy defeated - spawn projectile, defeat burst, and death animation
				if text.contains("defeated") and text.contains("gold"):
						...
						# Trigger death animation for defeated enemy
						var defeated_id: int = _get_last_defeated_enemy_id()
				...
				# Tower attack - trigger fire animation and hit flash
				elif text.begins_with("Tower hits"):
						# Extract enemy ID from "Tower hits kind#ID."
						var hash_pos: int = text.find("#")
						if hash_pos > 0:
								var end_pos: int = text.find(".", hash_pos)
								if end_pos > hash_pos:
										var enemy_id_str: String = text.substr(hash_pos + 1, end_pos - hash_pos - 1)
										var enemy_id: int = int(enemy_id_str)
										# Find enemy position for effects
										for enemy in state.enemies:
												if int(enemy.get("id", -1)) == enemy_id:
														var pos: Vector2i = enemy.get("pos", Vector2i.ZERO)
														grid_renderer.trigger_nearest_tower_fire(pos)
														grid_renderer.trigger_hit_flash(enemy_id)
														break
				# Exploration reveal
				elif text.begins_with("Discovered tile"):
						# Parse coordinates from "Discovered tile (x,y): terrain"
						var paren_start: int = text.find("(")
						var paren_end: int = text.find(")")
						if paren_start > 0 and paren_end > paren_start:
								var coords: String = text.substr(paren_start + 1, paren_end - paren_start - 1)
								var coord_parts: PackedStringArray = coords.split(",")
								if coord_parts.size() >= 2:
										var explore_x: int = int(coord_parts[0].strip_edges())
										var explore_y: int = int(coord_parts[1].strip_edges())
										grid_renderer.spawn_explore_reveal(Vector2i(explore_x, explore_y))
				# Building construction
				elif text.begins_with("Built "):
						# Trigger build effect at cursor position
						if state != null:
								grid_renderer.spawn_build_effect(state.cursor_pos)
				# Resource gains
				elif text.begins_with("Treasury yields"):
						grid_renderer.spawn_gold_sparkle()
				elif text.begins_with("Healers restore"):
						grid_renderer.spawn_heal_sparkle()
				elif text.begins_with("Gathered ") and text.contains("gold"):
						grid_renderer.spawn_gold_sparkle()
				# Castle damage
				elif text.begins_with("Enemy") and text.contains("hits the base"):
				...
				# Enemy spawn effect
						...
						# Parse coordinates from "Enemy spawned: kind#id at (x,y) ..."
						var at_pos: int = text.find(" at (")
						if at_pos > 0:
								var paren_end: int = text.find(")", at_pos)
								if paren_end > at_pos:
										var coords: String = text.substr(at_pos + 5, paren_end - at_pos - 5)
										var coord_parts: PackedStringArray = coords.split(",")
										if coord_parts.size() >= 2:
												var spawn_x: int = int(coord_parts[0].strip_edges())
												var spawn_y: int = int(coord_parts[1].strip_edges())
												grid_renderer.spawn_enemy_appear(Vector2i(spawn_x, spawn_y))
				# Wave/night start effect
				elif text.begins_with("Night falls"):
						grid_renderer.spawn_wave_start_effect()
				# Victory celebration
				elif text.begins_with("VICTORY"):
						grid_renderer.spawn_victory_effect()
				# Game over / defeat
				elif text.begins_with("Game over") or text.begins_with("The kingdom has fallen"):
						grid_renderer.spawn_defeat_effect()
				# Upgrade purchased
				elif text.begins_with("Upgraded") or text.begins_with("Purchased"):
						grid_renderer.spawn_upgrade_effect()
				# Dawn / wave complete
				elif text.begins_with("Dawn breaks"):
						grid_renderer.spawn_dawn_effect()
				# Boss encounter
				elif text.begins_with("BOSS ENCOUNTER:"):
						grid_renderer.spawn_boss_appear_effect()
				# POI appeared
				elif text.contains("point of interest"):
						# Trigger at cursor position since POIs spawn during exploration
						if state != null:
								grid_renderer.spawn_poi_effect(state.cursor_pos)
				# Blocked action tooltips
				elif _is_blocked_action_event(text):
						_spawn_action_tooltip(text)

## Check if an event text indicates a blocked action
func _is_blocked_action_event(text: String) -> bool:
	var lower := text.to_lower()
	return (lower.begins_with("not enough") or
			lower.begins_with("can't ") or
			lower.begins_with("cannot ") or
			lower.begins_with("invalid ") or
			lower.begins_with("no ") or
			lower.contains("requires ") or
			lower.contains("only during ") or
			lower.contains("already "))

## Spawn a floating tooltip for blocked actions
func _spawn_action_tooltip(reason: String) -> void:
	if ui_root == null:
		return
	# Position near the command bar or center of screen
	var viewport_size := get_viewport_rect().size
	var tooltip_pos := Vector2(viewport_size.x * 0.5, viewport_size.y * 0.7)
	if command_bar != null:
		tooltip_pos = command_bar.global_position + Vector2(command_bar.size.x * 0.5, -20)
	# Determine icon based on reason
	var icon := ""
	var lower := reason.to_lower()
	if lower.contains("gold") or lower.contains("resource"):
		icon = ""
	elif lower.contains("ap") or lower.contains("action point"):
		icon = ""
	elif lower.contains("phase") or lower.contains("night") or lower.contains("day"):
		icon = ""
	else:
		icon = ""
	ActionTooltipClass.spawn_at(ui_root, tooltip_pos, reason, icon)

## Check events for achievement triggers
func _trigger_achievement_events(events: Array) -> void:
	...
	for event in events:
		...
		# Enemy defeated
		if text.contains("defeated") and text.contains("gold"):
			var is_boss: bool = text.begins_with("BOSS DEFEATED:")
			var boss_kind: String = ""
			if is_boss:
				# Extract boss kind from event text like "BOSS DEFEATED: Void Tyrant"
				var parts: PackedStringArray = text.split(":")
				if parts.size() > 1:
					boss_kind = parts[1].strip_edges().to_lower().replace(" ", "_")
			var result: Dictionary = achievement_checker.on_enemy_defeated(profile, is_boss, boss_kind)
			profile = result.get("profile", profile)
...
## Get ID of last defeated enemy (focus or first candidate)
func _get_last_defeated_enemy_id() -> int:
		if typing_focus_id >= 0:
				return typing_focus_id
		if not typing_candidate_ids.is_empty():
				return int(typing_candidate_ids[0])
		return -1

## Get kind of last defeated enemy
func _get_last_defeated_enemy_kind() -> String:
		var target_id: int = _get_last_defeated_enemy_id()
		if target_id < 0:
				return "runner"
		# Check current enemies list
		for enemy in state.enemies:
				if int(enemy.get("id", -1)) == target_id:
						return str(enemy.get("kind", "runner"))
		# If not in current list (already removed), default
		return "runner"
...
func _load_profile() -> void:
	...
	profile = streak_result.get("profile", profile)
	if streak_result.get("changed", false):
		_persist_profile()
		var streak_val: int = int(streak_result.get("streak", 1))
		if streak_result.get("extended", false):
			if streak_val > 1:
				_append_log(["Daily streak extended to %d days!" % streak_val])
		elif streak_result.get("broken", false):
			_append_log(["Streak reset. Welcome back!"])
...
func _connect_settings_manager() -> void:
	if settings_manager == null:
		return
	# Connect to settings changed signal for accessibility sync
	if settings_manager.has_signal("settings_changed"):
		settings_manager.settings_changed.connect(_on_settings_manager_changed)
	# Initial sync from settings manager
	_sync_accessibility_settings()

func _on_settings_manager_changed() -> void:
	_sync_accessibility_settings()

func _sync_accessibility_settings() -> void:
	if settings_manager == null:
		return
	# Sync reduced motion
	if settings_manager.reduced_motion != reduced_motion:
		reduced_motion = settings_manager.reduced_motion
		if grid_renderer != null:
			grid_renderer.reduced_motion = reduced_motion
		# Also update profile
		var save_result: Dictionary = TypingProfile.set_reduced_motion(profile, reduced_motion)
		if save_result.get("ok", false):
			profile = save_result.get("profile", profile)
	# Sync high contrast
	if settings_manager.high_contrast != high_contrast:
		high_contrast = settings_manager.high_contrast
		var save_result: Dictionary = TypingProfile.set_high_contrast(profile, high_contrast)
		if save_result.get("ok", false):
			profile = save_result.get("profile", profile)
		_apply_contrast_theme()
...
func _record_typing_report(report: Dictionary) -> void:
	...
	_check_wave_achievements(report)
	_persist_profile()
	_refresh_history_panel()
	_refresh_trend_panel()
	_refresh_lesson_panel()
...
func _init_achievement_system() -> void:
	...
	# Create achievement popup and add to UI
	achievement_popup = ACHIEVEMENT_POPUP_SCENE.instantiate()
	if ui_root != null:
		ui_root.add_child(achievement_popup)
	else:
		add_child(achievement_popup)

## Handle achievement unlock event - show popup
...
## Check for combo milestone and show message
func _check_combo_milestone(prev_combo: int, new_combo: int) -> void:
	# Only check for major milestones (10, 20, 50)
	var milestones: Array[int] = [10, 20, 50]
	for milestone in milestones:
		if new_combo >= milestone and prev_combo < milestone:
			var message: String = StoryManager.get_combo_milestone_message(new_combo)
			if not message.is_empty():
				_append_log(["[color=yellow]COMBO x%d! %s[/color]" % [new_combo, message]])
			else:
				_append_log(["[color=yellow]COMBO x%d![/color]" % new_combo])
			# Play milestone audio stinger
			if audio_manager != null:
				if milestone >= 50:
					audio_manager.play_sfx(audio_manager.SFX.ACHIEVEMENT_UNLOCK)
				elif milestone >= 20:
					audio_manager.play_sfx(audio_manager.SFX.LEVEL_UP)
				else:
					audio_manager.play_sfx(audio_manager.SFX.TUTORIAL_DING)
			return

## Check wave completion for achievements
func _check_wave_achievements(report: Dictionary) -> void:
	if achievement_checker == null:
		return

	# Build stats dictionary for achievement checker
	var stats: Dictionary = {
		"accuracy": float(report.get("avg_accuracy", 0.0)),
		"wpm": float(report.get("wpm", 0.0)),
		"damage_taken": int(report.get("damage_taken", 0)),
		"hp_remaining": int(state.base_hp),
		"won": state.phase != "game_over",
		"best_combo": int(report.get("best_combo", 0))
	}

	# Check wave-related achievements
	var result: Dictionary = achievement_checker.on_wave_complete(profile, stats)
	profile = result.get("profile", profile)

	# Also check lesson mastery achievements
	var mastered: Array = []
	for lesson_id in lesson_progress.keys():
		var progress: Dictionary = lesson_progress.get(lesson_id, {})
		if int(progress.get("stars", 0)) >= 3:

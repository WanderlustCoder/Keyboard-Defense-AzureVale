...
func _build_ui() -> void:
	...
	var title := Label.new()
	title.text = "QUESTS"
	DesignSystem.style_label(title, "h2", Color(0.8, 0.6, 1.0))
...
func _build_types_section() -> void:
	...
	for quest_type in QUEST_TYPES:
		...
		DesignSystem.style_label(name_label, "caption", quest_type.get("color", Color.WHITE))
		name_label.custom_minimum_size = Vector2(110, 0)
		hbox.add_child(name_label)

		var refresh_label := Label.new()
		refresh_label.text = "[%s]" % quest_type.get("refresh", "")
		DesignSystem.style_label(refresh_label, "caption", ThemeColors.TEXT_DIM)
...
func _build_quest_list_section(title: String, color: Color, quests: Array[Dictionary]) -> void:
	...
	for quest in quests:
		var container := DesignSystem.create_hbox(DesignSystem.SPACE_XS)
		...
		DesignSystem.style_label(reward_label, "caption", ThemeColors.TEXT_DIM)
...
func _build_objectives_section() -> void:
	...
	for obj in OBJECTIVE_TYPES:
		...
		DesignSystem.style_label(name_label, "caption", obj.get("color", Color.WHITE))
...
func _build_tips_section() -> void:
	var section := _create_section_panel("QUEST TIPS", ThemeColors.SUCCESS)
	_content_vbox.add_child(section)

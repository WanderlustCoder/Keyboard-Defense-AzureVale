...
$logPath = Join-Path $projectRoot "_scenarios.log"
...
& $godot --headless --path . --script res://tools/run_scenarios.gd --log-file $logPath --tag p0 --tag balance --exclude-tag long --targets --out-dir $outDirArg 2>&1 | Tee-Object -FilePath $logPath | Out-Null

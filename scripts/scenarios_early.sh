...
LOG="${PROJECT_ROOT}/_scenarios_early.log"
...
"${GODOT}" --headless --path "${PROJECT_PATH}" --script res://tools/run_scenarios.gd --log-file "${LOG_PATH}" --tag p0 --tag balance --tag early --enforce-targets --out-dir "${OUT_DIR_ARG}" 2>&1 | tee "${LOG}" >/dev/null

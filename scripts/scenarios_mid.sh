...
LOG="${PROJECT_ROOT}/_scenarios_mid.log"
...
"${GODOT}" --headless --path "${PROJECT_PATH}" --script res://tools/run_scenarios.gd --log-file "${LOG_PATH}" --tag p0 --tag balance --tag mid --enforce-targets --out-dir "${OUT_DIR_ARG}" 2>&1 | tee "${LOG}" >/dev/null

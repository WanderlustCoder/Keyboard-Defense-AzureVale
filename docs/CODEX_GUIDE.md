...
| `node scripts/ci/goldBaselineGuard.mjs --timeline artifacts/summaries/gold-timeline.ci.json --baseline docs/codex_pack/fixtures/gold/gold-percentiles.baseline.json --out-json temp/gold-baseline-guard.json --mode warn` | Verifies every timeline scenario has a percentile baseline entry, writes a coverage report, and raises a warning when scenarios are missing baseline rows (mirrors the nightly guard step). |
...
| `node scripts/ci/goldTimelineDashboard.mjs docs/codex_pack/fixtures/gold-timeline/smoke.json --baseline docs/codex_pack/fixtures/gold/gold-percentiles.baseline.json --summary temp/gold-timeline.fixture.json --mode warn` | Produces the gold timeline dashboard summary (derived metrics + Markdown) matching the CI step and surfaces drift vs the committed percentile baseline. |
...
| `node scripts/ci/goldAnalyticsBoard.mjs --summary <file> --timeline <file> --passive <file> --percentile-guard <file> --percentile-alerts <file> --timeline-baseline docs/codex_pack/fixtures/gold/gold-percentiles.baseline.json --out temp/gold-analytics-board.fixture.json` | Aggregates the gold summary, timeline, passive, and percentile guard feeds into one Markdown/JSON bundle (now including starfield averages + per-scenario starfield notes). Includes baseline coverage warnings when scenarios are missing baseline entries (task `gold-analytics-board`). |
...
| `node scripts/hudScreenshots.mjs --ci --out artifacts/screenshots --starfield-scene tutorial` | Captures deterministic HUD/Tutorial screenshots (all six shots: hud-main, diagnostics-overlay, options-overlay, shortcut-overlay, tutorial-summary, wave-scorecard), forces the requested starfield tint (`tutorial|warning|breach`), and writes `*.meta.json` sidecars (condensed state, taunt details, starfield badges) for docs/hud_gallery.md + CI summaries. |
| `node scripts/docs/renderHudGallery.mjs --input artifacts/screenshots --output docs/hud_gallery.md --json artifacts/summaries/ui-snapshot-gallery.json --verify` | Rebuilds the HUD screenshot gallery, emits a JSON summary for dashboards, dedupes fixture/live metadata sources per shot, and fails when required screenshots/badges are missing. |
...
| `npx playwright test --config playwright.config.ts --project=visual --grep \"hud-main|diagnostics-overlay|options-overlay|shortcut-overlay|tutorial-summary|wave-scorecard\"` | Visual verification for HUD screenshots. |
| `npm run ci:download-artifacts -- --run-id <RUN_ID> --name ci-matrix-summary --name codex-dashboard-nightly` | Download CI/nightly artifacts via GitHub CLI into `artifacts/nightly-download` (requires `gh` auth). |
| `gh workflow run ci-matrix-nightly.yml --ref master` | Manually dispatch the nightly scenario matrix (asset integrity + HUD screenshots + condensed audit). |
| `gh workflow run codex-dashboard-nightly.yml --ref master` | Manually dispatch the nightly Codex dashboard/portal refresh (uses live CI artifacts when present, falls back to fixtures). |
...
### Baseline coverage (gold dashboards)

- If the gold analytics board warns about missing timeline baselines, regenerate timeline summaries with `--baseline docs/codex_pack/fixtures/gold/gold-percentiles.baseline.json` (or your refreshed baseline) and rerun `goldAnalyticsBoard.mjs` with `--timeline-baseline <same file>`.
- Refresh percentile baselines via `npm run gold:percentiles:refresh` (or `node scripts/ci/goldPercentileBaseline.mjs --out docs/codex_pack/fixtures/gold/gold-percentiles.baseline.json`) when new scenarios land so baseline coverage returns to 100%. After updating, rerun `npm run codex:dashboard`.

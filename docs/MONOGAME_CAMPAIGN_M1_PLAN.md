...
1. `CM1-001` Campaign node profile binding
...
2. `CM1-002` Battle -> summary handoff contract
...
3. `CM1-003` Campaign progression application
...
- [ ] Campaign-node victory grants completion + reward once.
- [ ] Repeat victory on same node does not grant additional node reward.
- [ ] Defeat does not grant node reward and keeps retry messaging clear.
- [ ] Summary messaging is derived from shared campaign outcome state.
- [ ] Test suite covers node mapping, handoff, and reward semantics.

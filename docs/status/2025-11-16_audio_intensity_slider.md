...
**Next Steps**
1. Mirror the intensity control inside the debug panel (`debug-sound-controls`) so engineers can tweak both volume and intensity without opening the pause menu.
2. Consider exposing intensity metrics in smoke artifacts/telemetry so we can correlate comfort settings with combo retention or session length.
